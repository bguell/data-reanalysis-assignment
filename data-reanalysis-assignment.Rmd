...
library(stargazer)
kable(ll,title="Mean, SD, & Range for frog species",digits=3)
```
...
gc= gc + labs(x= "Age (days)", y= "Sponanteous hatching \n (proportion hatched eggs)") #make labels for axis
...
gc= ggplot(data=spc, aes(x=age, y=mean_survival)) + geom_bar(stat="identity", fill= "black") #use barplots w/age filled all black
gc= gc + geom_errorbar(aes(ymin=mean_survival-SE, ymax=mean_survival+SE), width=.2, position=position_dodge(.9), color="black") #black error bars
gc= gc + labs(x= "Age (days)", y= "Escape hatching success \n (proportion hatched eggs)") #make labels for axis
gc= gc + theme_bw(base_size = 11, base_family = "Times") #make theme in Times New Roman and b/w
gc= gc + annotate("text", x = 4.8, y = 1, label = "(b) A. callidryas", fontface= "italic") #add italicized text (species name) to figure!
gc= gc + scale_y_continuous(breaks= pretty_breaks(n=5)) # allows for making "pretty" breaks on y axis... shows 5 breaks
gc= gc + geom_text(aes(y = 0.03, label = spc$sample_size), color= "white", vjust = 0) # add white numbers of the sample sizes for each age
gc

#and again same for spurrelli
gs= ggplot(data=sps, aes(x=age, y=mean_survival)) + geom_bar(stat="identity", fill= "black")
gs= gs + geom_errorbar(aes(ymin=mean_survival-SE, ymax=mean_survival+SE), width=.2, position=position_dodge(.9), color="black") 
gs= gs + labs(x= "Age (days)", y= "Escape hatching success \n (proportion hatched eggs)") 
gs= gs + theme_bw(base_size = 11, base_family = "Times")
gs= gs + annotate("text", x = 3.65, y = 1, label = "(a) A. spurrelli", fontface= "italic")
gs= gs + scale_x_continuous(breaks= pretty_breaks(n=2)) # allows for making "pretty" breaks on x axis... only showing 4 and 5 values...
gs= gs + scale_y_continuous(breaks= pretty_breaks(n=5)) # allows for making "pretty" breaks on y axis... shows 5 breaks
gs= gs + geom_text(aes(y = 0.03, label = sps$sample_size), color= "white", vjust = 0) # add white numbers of the sample sizes for each age
gs
...
gc= gc + geom_errorbar(aes(ymin=mean_survival-SE, ymax=mean_survival+SE), width=.2, position=position_dodge(.9), color="black") #black error bars
gc= gc + labs(x= "Age (days)", y= "Hatching survival \n (proportion)") #make labels for axis
gc= gc + theme_bw(base_size = 11, base_family = "Times") #make theme in Times New Roman and b/w
gc= gc + annotate("text", x = 4.8, y = 0.80, label = "(b) A. callidryas", fontface= "italic") #add italicized text (species name) to figure!
gc= gc + scale_y_continuous(breaks= pretty_breaks(n=5)) # allows for making "pretty" breaks on y axis... shows 5 breaks
gc= gc + geom_text(aes(y = 0.03, label = sample_size), color= "white", vjust = 0) # add white numbers of the sample sizes for each age
gc

#and again same for spurrelli
gs= ggplot(data=fps, aes(x=age, y=mean_survival)) + geom_bar(stat="identity", fill= "black")
gs= gs + geom_errorbar(aes(ymin=mean_survival-SE, ymax=mean_survival+SE), width=.2, position=position_dodge(.9), color="black") 
gs= gs + labs(x= "Age (days)", y= "Hatching survival \n (proportion)") 
gs= gs + theme_bw(base_size = 11, base_family = "Times")
gs= gs + annotate("text", x = 5.65, y = 0.80, label = "(a) A. spurrelli", fontface= "italic")
gs= gs + scale_x_continuous(breaks= pretty_breaks(n=2)) # allows for making "pretty" breaks on x axis... only showing 4 and 5 values...
gs= gs + scale_y_continuous(breaks= pretty_breaks(n=5)) # allows for making "pretty" breaks on y axis... shows 5 breaks
gs= gs + geom_text(aes(y = 0.03, label = sample_size), color= "white", vjust = 0) # add white numbers of the sample sizes for each age
gs
